	...
	*/
	...
	try
	{
		...
		for(map<string,map<string, unsigned int> >::iterator nfpnos = networkFunctionsPortsNameOnSwitch.begin(); nfpnos != networkFunctionsPortsNameOnSwitch.end(); nfpnos++) {
			...
			map<string, unsigned int> names = nfpnos->second;
			for (map<string, unsigned int>::iterator n_it = names.begin(); n_it != names.end(); ++n_it) {
				SwitchPortsAssociation::setAssociation(graph->getID(), n_it->first, nfpnos->first);
			}
#endif
	...
	for(list<highlevel::VNFs>::iterator nf = network_functions.begin(); nf != network_functions.end(); nf++)
	{
		...
		try
		{
			...
			for(list<string>::iterator port = ports.begin(); port != ports.end(); port++)
				SwitchPortsAssociation::setAssociation(graphID, *port, anpo->getNfId());
#endif
