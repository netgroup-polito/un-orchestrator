...
bool Docker::startNF(StartNFIn sni)
{
	...
	string datadisk_dst_path = sni.getDatadiskDstPath();
#else
	string dir_to_mount = "None";
	string datadisk_dst_path = "None";
#endif
	...
	ULOG_DBG_INFO("datadisk_dest_path: %s", datadisk_dst_path.c_str());
#endif
	command << getenv("un_script_path") << PULL_AND_RUN_DOCKER_NF << " " << lsiID << " " << nf_id << " " << uri_image << " " << nf_name << " " << uri_type << " " << dir_to_mount << " " << datadisk_dst_path << " " << n_ports ;
